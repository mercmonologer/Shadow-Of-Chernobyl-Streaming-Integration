...
static MySocketServer server;      // Ensure a single instance
static std::thread socketThread;   // Store the thread globally
static std::atomic<bool> threadRunning = false; // Track if the thread is running
...
BOOL CActor::net_Spawn(CSE_Abstract* DC)
{
    ...
    */
    ...
    if (!server.isRunning() && !threadRunning) {  
        threadRunning = true;  // Mark thread as started

        socketThread = std::thread([]() {
            server.start(12345);
            threadRunning = false; // Reset if server stops
        });

        socketThread.detach(); // Allow it to run independently
    }
